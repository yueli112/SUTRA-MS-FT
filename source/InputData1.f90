...
!........LOAD APPROPRIATE TEXT INTO TITLE FOR ".nod" FILE(fNOD)          
         ...
         IF(CH_K5COL(M) .EQ.'L') CSMSK5(MONSPE) = VARNK5(8)    !冰水
         IF(CH_K5COL(M) .EQ.'I') CSMSK5(MONSPE) = VARNK5(9)     !    
         
         IF(CH_K5COL(M) .EQ.'R') CSMSK5(MONSPE) = VARNK5(10) 
!........VERSION 1.1
         IF(CH_K5COL(M) .EQ.'H') CSMSK5(MONSPE) = VARNK5(11) 
         IF(CH_K5COL(M) .EQ.'V') CSMSK5(MONSPE) = VARNK5(12) 
...
!........TEST FOR PRESENCE OF P, U, SW, RHO, OR TOTAL HEAD FOR TECPLOT                     
         ...
         IF(CH_K5COL(M) .EQ.'S') LTpSW                    = .TRUE. 
       
         IF(CH_K5COL(M) .EQ.'L')                   LTpSL                   = .TRUE.          !!在MODULE JCOLS改了名称和最大数量
         IF(CH_K5COL(M) .EQ.'I')                   LTpSI                   = .TRUE.
...
!.....VERSION 1.1
!.....RESET NCOLS5 IF OUTPUTTING RHO, TOTAL HEAD, AND/OR VELOCITY                     冰水需要设置这个-1吗？？
      ...
      DO 2250 M = 1, NCOLS6 
         DO 2200 MM = 1, 10                             !冻结7改10
            IF (K6COL (M) .EQ.K6SYM (MM) ) THEN 
               ...
               ENDIF 
               IF ( (MM.EQ.10) .AND. (IABS (KTYPE) .EQ.2) ) THEN                       !!冻结达西速度
                  call ErrorIO('Error CANNOT PRINT "qZ" FOR A 2-D PROBLEM')            !
               ENDIF 
               J6COL (M) = MM 
               GOTO 2250 
            ENDIF 
...
!                                                                       
!.....INPUT DATASET 8D: OUTPUT CONTROLS AND OPTIONS -- UNIT fOBS               还没改
      if(fSOB>0) goto 4399          
...
!.....SET SIGMAW, URHOW0, DRWDU, AND VISC0 TO BASE VALUES           所以BVISC0没用    
      SIGMAW = BSIGMAW 
      URHOW0 = BURHOW0 
      DRWDU = BDRWDU 
      VISC0 = 0.00D0           !!粘度读了，但先设0了
...
!.....IF ENERGY TRANSPORT ONLY                                          
      IF (ME.EQ.1) THEN 
         VISC0 (NESP) = BVISC0            !!仅模拟温度时，输入的比例因子会赋给visco
         BVISC0 = 0.0D0 
      ENDIF 
!                                                                       
...
!
!.....Set SOP from zones
      if(.not.MkZoneSOP(COMPFL,COMPI)) call ErrorIO('Error specifying SOP for nodal zones')    !对应加了COMPI
!                                                                       
!.....INITIALIZE ADSMOD TO NONE AND ADSORPTION COEFFICIENTS TO ZERO                            !!!!!  吸附相关量还没有改
      ADSMOD = 'NONE      ' 
      CHI1 = 0.0D0 
      CHI2 = 0.0D0 
...
!.....INITIALIZE PRODUCTION OF ENERGY AND SOLUTE MASS VARIABLES         
      ...
      PRODF1 = 0.0D0 
      PRODS1 = 0.0D0 
!.....INPUT DATASET 12: PRODUCTION OF ENERGY OR SOLUTE MASS       不考虑自我生产项，没改      
...
!           SET SPECIFIC PRESSURE STORATIVITY, SOP和SOPI.                           
            if(fZON==0) NodeData(NodeMap(ii))%sop = (1.D0 - NodeData(NodeMap(ii))%por ) * NodeData(NodeMap(ii))%compma + NodeData(NodeMap(ii))%por * COMPFL
            if(fZON==0) NodeData(NodeMap(ii))%sopi = (1.D0 - NodeData(NodeMap(ii))%por ) * NodeData(NodeMap(ii))%compma + NodeData(NodeMap(ii))%por * COMPI    !!!
...
!.....INPUT DATASET 14D: PRODUCTION PARAMETERS SPECIFIED BY NODE
      if( fZON /= 0 ) then       !!也改了，但这个等不等于好像不影响
