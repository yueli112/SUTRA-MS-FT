...
!
 !       WRITE(*,*) '计算',KSOLVR,R(2485),XITER (2485)    !!XITER已经算出结果了
...
!.......COPY THE RHS VECTOR "R" INTO "B", THEN USE "R" AS THE SLAP        将右手边向量 "R" 复制到 "B"，再用R来求解
...
!WRITE(*,*)'SOLVER前',B(1),R(1)
!        DO I = 1, NNP
!IF (KSP.NE.NESP) THEN                                   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!增加结冰处浓度
!   IF (IT.LE.1) THEN
!     SLL1(I)=SLL(I)
!  END IF 
!   R(I)  = R(I)*SLL1(I)/SLL(I)
!  ! UITER(I,K)  = UITER(I,K)*2
!END IF    
!    END DO   
 ...
 !WRITE(*,*)'SOLVER后',B(1),R(1)    !!R有变化成结果，B不变
!                                                                       
      !ATSDATA MODULE            
