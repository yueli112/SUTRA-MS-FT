...
!                                                                       
      SUBROUTINE OUTVELOCITY(VMAG,VDMAG, VANG1, VANG2, IN, X, Y, Z, TITLE1, TITLE2) 
      ...
      real (DP) :: &
        VMAG (NE),VDMAG(NE), VANG1 (NE), VANG2 (NEX) 
      real (DP) :: &
        X (NN), Y (NN), Z (NN) 
      real (DP) :: &
        VCOL (NCOLMX), VVAR (10) 
      CHARACTER(1) TITLE1 (80), TITLE2 (80) 
      CHARACTER(15) COLTK6 (10) 
      CHARACTER(1) CPVX, CPVY, CPVZ ,CPQX,CPQY,CPQZ
      integer (I4B) :: &
        IN (NIN), IIN (8)
      real (DP) :: &
        TT (999999)
      ...
      REAL (DP) :: &
        ...
        RN48, &
        TS, &
        VA1, VA2, VECTRX, VECTRY, VECTRZ,VECTDX,VECTDY,VECTDZ


      DATA (COLTK6 (MM) , MM = 1, 10)  / 'Element', &
          '       X origin', '       Y origin', '       Z origin', &
          '     X velocity', '     Y velocity', '     Z velocity' ,&                                       
        '     X DARCYvel', '     Y DARCYvel', '     Z DARCYvel' /
      SAVE COLTK6 
...
!.....   Indicators of when variables are calculated and printed        
         ...
         DO 8 M = 1, NCOLS6 
            ...
            IF (J6COL (M) .EQ.7) CPVZ = 'Y' 
                IF (J6COL(M).EQ.8) CPQX = 'Y'   
                IF (J6COL(M).EQ.9) CPQY = 'Y'   
                IF (J6COL(M).EQ.10) CPQZ = 'Y'          !!!!
    8    END DO 
   10    CONTINUE 
         JT = JT + 1 
...
!.....Write the header information                                      
         IF (fELE.GT.0) THEN 
            ...
            ENDIF 
            WRITE (fELE, 964) "VELOCITYRESULTS", KTMAX, "Vx", "Vy", "Vz" , "qx", "qy", "qz"               !!!
            DO 20 KT = 1, KTMAX 
               WRITE (fELE, 965) ITT (KT), TT (KT), CPVX, ISVEL (KT),     &
               CPVY, ISVEL (KT), CPVZ, ISVEL (KT)  , CPQX, ISVEL(KT), CPQY, ISVEL(KT), CPQZ, ISVEL(KT)                   !!!
   20       END DO 
         ENDIF 
  960 FORMAT   ("## ", 80A1,                                            &
     &         /"## ", 80A1,                                            &
  ...
  963 FORMAT   ("## ", I1, "-D, IRREGULAR MESH", 40X, I9, A,            &
     &         /"## ")                                                  
964  FORMAT ("## ", 123("="),                                       &
      &     /"## ", A, 59X, I9, " Time steps printed",               &
      &     /"## ", 123("="),                                        &
      &     /"## ", 'basic units: [L] is distance unit (e.g., [m])', &
      &            ' and [s] is time in seconds',                    &
      &     /"## ", '  fluid velocity units [L/s]',                  &
      &     /"## ", 'volumetric flux: fluid volume [L^3] flowing ',  &
      &            'through unit cross-sectional area [L^2] each [s]'&
      &     /"## ", '  volumetric flux ("Darcy velocity") ',         &
      &            'units [(L^3/s)/L^2]',                            &
      &     /"## ", 123("="),                                        &
      &     /"## ",                                                  &
      &     /"## ", 4X, "Time steps", 42X,                           &
      &             "[Printed? / Time step on which V or q is based]"&
      &     /"## ", 3X, "in this file      Time (sec)",              &
      &             10X,A2, 5(13X,A2)                                &
      &     /"## ", 2X, 14("-"), 3X, 13("-"), 1X, 6(3X, 12("-")) )   
  ...
  965 FORMAT    ("## ", 3X, I8, 3X, 1PE13.6, 6(5X, A1, 1X, I8)) 
...
!.....  The velocity data for this time step                            
   ...
   40       END DO 
            ...
            VECTRZ = VMAG (L) * DSIN (VA2)
              ...
              VECTDX=VDMAG(L) * DCOS(VA1)*CVA2          
              VECTDY=VDMAG(L) * DSIN(VA1)*CVA2          
              VECTDZ=VDMAG(L) * DSIN(VA2)               !!!
