...
!                                                                       
      MODULE PARAMS 
        ...
        LOGICAL :: LVolAvgLambda = .true.    !!本来是默认true                  false
...
!                                                                       
      MODULE JCOLS
        USE SutraMSPrecision
        INTEGER (I4B), PARAMETER :: K5MAX=12  !8 CHANGED TO 10 - VERSION 1.1 冻结10改12
        INTEGER (I4B), PARAMETER :: K6MAX=10   !冻结7改10
        ...
        DATA K5SYM / 'N', 'X', 'Y', 'Z', 'P', 'U', 'S','L','I', 'R', 'H', 'V' /                    !加L I
        DATA VARNK5 / 'NODE NUMBER', 'X-COORDINATE', 'Y-COORDINATE', 'Z-COORDINATE', &
                      'PRESSURE/HEAD', 'CONCENTRATION/TEMPERATURE', &
                      'TOTAL SATURATION', 'LIQUID SATURATION', 'ICE SATURATION', &
                      'DENSITY', 'TOTAL HEAD', 'NODAL VELOCITY' /                                                             
       
        DATA K6SYM / 'E', 'X', 'Y', 'Z', 'VX', 'VY', 'VZ' , 'qX', 'qY', 'qZ' /                 !加
        DATA VARNK6 / 'ELEMENT NUMBER', 'X-COORDINATE OF CENTROID', 'Y-COORDINATE OF CENTROID', &
                      ...
                      'X-DarcyVELOCITY', 'Y-DarcyVELOCITY', 'Z-DarcyVELOCITY'/                                        
        PUBLIC :: &
...
!    WATER RETENTION PARAMETERS USED AT UNSAT
      MODULE M_SWCC
         IMPLICIT NONE
         DOUBLE PRECISION :: SWRES1,AA1,VN1,SWRES2,AA2,VN2,SWRES3,AA3,VN3
      END MODULE M_SWCC
 ...
 ! ************************** yu   *************************************
      ...
      USE DIMS
      implicit none
      real POR1,SFSTAGE,voyu,EXCHW,BCEXC
      REAL(DP),dimension(:),allocatable :: TSAIR,HFPFLUX,NETRn,Uwind,Hraair,RAINYU, &
                           Sarea,ETP0,TDT,TDH
      REAL(DP),dimension(:),allocatable :: SWYU,TSSOIL,SSSOIL                           
!      REAL(DP),dimension(:),allocatable :: XET,YET,ZET
      REAL(DP),dimension(:),allocatable :: MEANP,MEANT,MEANC,MEANS,MEANVX,MEANVY,MEANVZ,XNT,ZNT,YNT,  &
                                           MEANSL,MEANSI                                   

      Public MEANP,MEANT,MEANC,MEANS,SWYU,TSSOIL,TSAIR,HFPFLUX,NETRn,Uwind,Hraair,  &
             RAINYU,Sarea,SSSOIL,ETP0,TDT,TDH,MEANVX,MEANVY,MEANVZ,XNT,ZNT,YNT,     &
             MEANSL,MEANSI
      common POR1
 ...
 !************************************************************************   
     ! ************************** !yu - 07142020 *****************************
       MODULE M_OUT
         IMPLICIT NONE
         INTEGER ::IFEVAP,IFSUCTION,BOUNDSPE,DM1,DM2, COLYU, S_OUT, OUT_INTE,ITtot
        REAL  ::DEV
    END MODULE M_OUT
 !************************************************************************      
      module surface
     USE SutraMSPrecision
      USE DIMS
      implicit none
      REAL(DP),dimension(:),allocatable :: SURTEM1
     Public  SURTEM1
     !
    END module surface
