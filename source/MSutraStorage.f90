     ...
     module SutraStorage
       ...
       !standard SUTRA dimensions
         integer (I4B), allocatable :: &
           IQSOP(:),  IPBC (:),    IN (:), &
           IFREEZE1(:),  IFREEZE2(:),UMMIN(:)                       !!yu
         real (DP), allocatable :: &
             QIN(:),    PBC(:), QPLITR(:), &
               X(:),      Y(:),      Z(:), &
              SW(:),  DSWDP(:),    RHO(:),RHO1(:),RHOCHANGE(:),UCHANGE(:),UCHANGE1(:),CTRNQ1(:), QIN1(:),VOL1(:), SLL(:), SLL1(:), SII1(:),SII2(:),CFREEZ(:),SW1(:),DSLDP(:),DSLDT(:),SII(:),  DSIDP(:),DSIDT(:), &       !!!
              SPACE(:) ,SIOUT(:),SIOUT1(:) ,SIOUT2(:),SPA(:) ,EXTRA(:), &
            PVEL(:), &
            VMAG(:),  VANG1(:),  VANG2(:), &
           VDMAG(:),&                                        !!!
          GXSI(:,:), GETA(:,:), GZET(:,:), &
       ...
       !ONLY ALLOCATED IF THE AUTOMATIC TIMESTEP ALGORITHM IS USED       
         ...
         public &
           ...
           QIN, PBC, QPLITR, &
           X, Y, Z, SW, DSWDP, RHO,RHO1,RHOCHANGE,UCHANGE,QIN1,VOL1,UCHANGE1,CTRNQ1,  &
           PVEL, &
           VMAG, VANG1, VANG2,VDMAG ,&           !!!
           GXSI, GETA, GZET, &
           VOL, PMAT, PVEC, UMAT, &
           PM1, PITER, RCIT, RCITM1, &
           ...
           QPLITR_, &
!
           SLL,SLL1,SII1,SII2,CFREEZ,SW1, SPACE,SIOUT,SIOUT1,SIOUT2,SPA,EXTRA, DSLDP,DSLDT,SII,  DSIDP,DSIDT,&                  !!!
           SLL_,SII_ ,IFREEZE1, IFREEZE2,UMMIN, &
           ...
           contains
             ...
             logical function AllocateInitialStorage()
               ...
               lOk=.false.
                ...
                NCOLMX = 12 + NSPE-1 !for JCOLS - 9 CHANGED TO 10 FOR VERSION 1.1    !!
                ...
                allocate ( &
                          ...
                          DRWDU(NSPE+1), SIGMAW(NSPE), URHOW0(NSPE), & 
                          CHI1(NSPE), CHI2(NSPE), & 
                          ATSPMULT(NSPE), & 
                          NSOU(NSPE), NUBC(NSPE), & 
                          !CONTRL
...
!
                  ...
                  X(NN), Y(NN), Z(NN), SW(NN), DSWDP(NN), RHO(NN),RHO1(NN),RHOCHANGE(NN),UCHANGE(NN),QIN1(NN),VOL1(NN),UCHANGE1(NN),CTRNQ1(NN),   &
                  PVEL(NN), &
                SLL(NN), SLL1(NN),SII1(NN),SII2(NN),CFREEZ(NN),SW1(NN),SPACE(NN),SIOUT(NN),SIOUT1(NN),SIOUT2(NN),SPA(NN),EXTRA(NN), DSLDP(NN),DSLDT(NN),SII(NN),  DSIDP(NN),DSIDT(NN),&                               !!!
                  VMAG(NE), VANG1(NE), VANG2(NEX), VDMAG(NE),IFREEZE1(NN),IFREEZE2(NN),UMMIN(NN),  &
...
!
                  ...
                  QUIN(NN, NSPE), UVEC(NN, NSPE+1), CVEC(NN), CITER(NN), &
                  UBC(NBCN, NSPE), &
                  RU(NSPE),&
                !end SMS DIMENSIONS                                                   
                    stat=ios) 
                ...
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !SII(NN)        freeze
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !SII(NN)
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NN) !
                
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NE) !VMAG(NE)         
                ios=AddMemory(MemoryIndex('SUT'),'Vec',        NE) !VDMAG(NE)            !!darcy
                ...
                if (fATS.GT.0 .and. .not.LContinue) then
                  ...
                  ios=AddMemory(MemoryIndex('ATS'),'Vec',        NN) !SII_(NN)                 
                  ios=AddMemory(MemoryIndex('ATS'),'Vec',        NN) !SLL_(NN)
