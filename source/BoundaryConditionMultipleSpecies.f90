...
!         READ (fINP, * ) K, IUBC (IUT, K), UBC (IUT, K) 
      ...
      ELSEIF (IUBC (IUT, K) .LT.0) THEN 
         IUBCT = - 1
         !set MultiSpeciesBC(K)%SpecifiedU(IU(K))%node for user programmed boundary conditions - Xinghua-20230418 bug fixed
         MultiSpeciesBC(K)%SpecifiedU(IU(K))%node=IUBC (IUT, K)
      ENDIF 
