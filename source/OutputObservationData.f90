...
!                                                                       
      SUBROUTINE OUTOBS (IOBS, X, Y, Z, PVEC, UVEC, SW,SLL,SII, TITLE1, TITLE2) 
      ...
      real (DP) :: &
        TT (9999)
      integer (I4B) :: &
        ITT (9999), ISTORC (9999), ISHORP (9999), ISSATU (9999),ISSATL(9999),ISSATI(9999)            !!
...
!.....Write the header information                                      
         IF (fOBS.GT.0) THEN 
            ...
            (TORC (K), K = 1, NSPE), "ToSat" , "LiqSat", "IceSat"                !!                             
            DO 20 KT = 1, KTMAX 
               WRITE (fOBS, C965) ITT (KT), TT (KT), ISHORP (KT), (ISTORC &
               (KT), K = 1, NSPE), ISSATU (KT)  , ISSATL(KT) ,  ISSATI(KT)           !!            
   20       END DO 
            NOBS = NOBSN - 1 
...
!                                                                       
      IF (fOBS.GT.0) THEN 
         WRITE(fOBS, C980)IT, TSEC,(PVEC(IOBS(JJ)),(UVEC(IOBS(JJ),K), K = 1, NSPE),&
                 SW(IOBS(JJ)),SLL(IOBS(JJ)),SII(IOBS(JJ)), JJ = 1, NOBS)                    !!
!                                                                       
      ENDIF                                                                  
      RETURN 
!                                                                       
