...
!                                                                       
      ...
      USE SutraZoneModule
      USE SutraStorage, ONLY: IN, X, Y, THICK=>Z, PITER, UITER, RCIT, RCITM1, VDMAG,&         !!
      ...
      real (DP) :: &
        ...
        DWDXG (4, 4), DWDYG (4, 4)                                        
      real (DP) :: &
        SWG (4),SLLG(4),SIIG(4),SIOUTG(4),SII1G(4),CFREEZG(4), RHOG (4), VISCG (4), PORG (4), VXG (4),        &
...
!      OPEN(21184,FILE='VISCOSITY.DAT')   ! PuLi-20200103
!      OPEN(21183,FILE='DENSITY.DAT')     ! PuLi-20200103 
!
!                                                                       
      ...
      DO 1000 L = 1, NE 
         DO 500 IL = 1, 4 
            ...
            CALL BASIS2 (0000, L, XLOC, YLOC, F (1, IL),      &
                         ...
                         VXG (IL), VYG (IL), SWG (IL),SLLG(IL),SIIG(IL),SIOUTG(IL),SII1G(IL),CFREEZG(IL), RHOG (IL), VISCG (IL),   &
         ...
         DO 2200 IYL = 1, 2 
            DO 2100 IXL = 1, 2 
               ...
               CALL BASIS2 (0001, L, XLOC, YLOC, F (1, KG),   &
                            ...
                            THICKG (KG), VXG (KG), VYG (KG), SWG (KG), SLLG(KG),SIIG(KG),SIOUTG(KG), SII1G(KG),CFREEZG(KG),      &
               ...
               !     
!               IF ((L.EQ.10000).AND.(IXL.EQ.2).AND.(IYL.EQ.2)) THEN
            !      WRITE ( * , * )'ELEMENT', L,SIOUTG(KG)
!                  write(20240723,'(10E15.6e3)') RELKG (KG)   
!            END IF          
            !   
...
!===================================================================== PuLi
!         IF (IT==ITMAX) THEN
!            WRITE (21184,'(4E16.5)') VISCG  !PuLi-20200103
!            WRITE (21183,'(4E16.5)') RHOG   !PuLi-20200103
!         ENDIF 
!===================================================================== PuLi         
!                                                                       
!.....CALCULATE VELOCITY AT ELEMENT CENTROID WHEN REQUIRED              
 ...
 2300    AXSUM = 0.0D0 
         ...
         AYSUMD=0.0D0    
            ...
            RELKI = 1         
         DO 2400 KG = 1, 4 
             ESLLG=PORG(KG)*SLLG(KG)            !!
            AXSUMD=AXSUMD+VXG(KG)*ESLLG 
            AYSUMD=AYSUMD+VYG(KG)*ESLLG          !!
            AXSUM = AXSUM + VXG (KG)              
            AYSUM = AYSUM + VYG (KG)

            IF (RELKG(KG) .EQ. 0) THEN 
                RELKI = 0
            ENDIF          
 2400    CONTINUE 
            IF ( RELKI .EQ. 0 ) THEN 
            ...
            ENDIF  
!            IF (L .EQ. 10000) THEN
!            write(20240723,'(3I,10E15.6e3)')  L,KG,RELKI,AXSUM,AYSUM,AXSUMD,AYSUMD
!            ENDIF             
         VDMAG(L)=DSQRT(AXSUMD*AXSUMD+AYSUMD*AYSUMD)      !!
         VMAG (L) = DSQRT (AXSUM * AXSUM + AYSUM * AYSUM) 
         IF (VMAG (L) .NE.0D0) THEN 
            VMAG (L) = VMAG (L) * 2.5D-1                               !!
             VDMAG(L)=VDMAG(L)*2.5D-1 
...
!        ELEMENT USING SYMMETRIC WEIGHTING FUNCTIONS                    
 5200    DO 6000 I = 1, 4 
            ...
            DO 5400 KG = 1, 4      
            VO = VO + F (I, KG) * DET (KG) 
            DF = DF + ( (RXXG (KG) * RGXG (KG) + RXYG (KG) * RGYG (KG) )&
 ...
 5400        CONTINUE   
            ...
            DO 5800 J = 1, 4 
               ...
               BF = BF + ( (RXXG (KG) * DFDXG (J, KG) + RXYG (KG)       &
 ...
 5800       BFLOWE (I, J) = BF 
           ...
           DFLOWE (I) = DF   
            ...
            ENDIF               
!   IF (L .EQ. 10000) THEN
!            write(20240723,'(3I,10E15.6e3)')  I,KG,RELKI,DFLOWE (I)
!   ENDIF 
 6000      CONTINUE    
 6200      CONTINUE 
...
!..........FOR ENERGY TRANSPORT:                                        
               if(LVolAvgLambda) then
                 ESE = ESLLG * SIGMAW(1) +( ESIIG+PORG (KG) * SIOUTG (KG) ) * SIGMAI + (1D0 - PORG (KG) ) * ElemData(ElemMap(L))%lambdas  !Volumetric Average
               else
                 !lambda(bulk) = (la^(1-SW) * lw^Sw)^por * ls(1-por)
                 !lambda(air)  = 0.026 W/(m degC) - parameter in PARAMS
                 ESE = (SIGMAI**(SIIG(KG)+SIOUTG (KG)) * SIGMAW(1)**SLLG(KG))**PORG(KG) * (ElemData(ElemMap(L))%lambdas**(1D0-PORG(KG)))       !Geometric Mean
               end if
            ELSE 
    ...
    !..........FOR SOLUTE TRANSPORT:                                        
            ...
            !IF (KSP.NE.NESP) THEN 
            ! IF((KG.EQ.1).AND.(L.EQ.3011))THEN
            !     WRITE(*,*) L,'ESE=',ESE,'SIIG=',SIIG(KG),'KUOSAN=',SIGMAW
            !   ENDIF
            !     IF((KG.EQ.1).AND.(L.EQ.1000))THEN
            !     WRITE(*,*) L,'ESE=',ESE,'SIIG=',SIIG(KG),'KUOSAN=',SIGMAW  
            ! END IF  
            ! END IF      
