...
!                                                                       
      ...
      real (DP) :: &
        SWG (8),SIIG(8),SLLG(8), SIOUTG(8),SII1G(8),CFREEZG(8),RHOG (8), VISCG (8), PORG (8), VXG (8), VYG (8), VZG (8), &          !!
      ...
      real (DP) :: &
        ...
        XIX, YIY, ZIZ, &
        AXSUM, AYSUM, AZSUM ,AXSUMD,AYSUMD,AZSUMD, &            !!!
        SWTEST, ROMG, &
        DF, VO, BF, &
        ESLLG,ESIIG, RHOCWG, ESRCG, &
        DXXG, DXYG, DXZG, &
        DYXG, DYYG, DYZG, &
        DZXG, DZYG, DZZG, &
        ALEFF, AT1EFF, AT2EFF, &
...
!
!      OPEN(234171,FILE='VISCOSITY.DAT')   ! Xinghua-20230417
!      OPEN(234172,FILE='DENSITY.DAT')     ! Xinghua-20230417 
      ...
      DO 1000 L = 1, NE 
         DO 500 IL = 1, 8 
            ...
            CALL BASIS3 (0000, L, XLOC, YLOC, ZLOC, F (1,IL), &
                         ...
                         SWG (IL),SLLG(IL),SIIG(IL),SIOUTG(IL),SII1G(IL),CFREEZG(IL), RHOG (IL), VISCG (IL), PORG (IL), VGMAG (IL),     &
                         RELKG (IL), &
            ...
            DO 2200 IYL = 1, 2 
               DO 2100 IXL = 1, 2 
                  ...
                  CALL BASIS3 (0001, L, XLOC, YLOC, ZLOC,  &
                               ...
                               DWDZG (1, KG), VXG (KG),&
                               VYG (KG), VZG (KG), SWG (KG),SLLG(KG),SIIG(KG),SIOUTG(IL),SII1G(IL),CFREEZG(IL), RHOG (KG), VISCG (KG),  &
                               PORG (KG), VGMAG (KG), RELKG (KG), &
                               CJ11, CJ12, CJ13, CJ21, CJ22, CJ23, CJ31,     &
                               CJ32, CJ33, RGXG (KG), RGYG (KG), RGZG (KG))
 
            !      
            !   IF ((L.EQ.2441).AND.(IXL.EQ.1).AND.(IYL.EQ.1).AND.(IZL.EQ.1)) THEN
            !       WRITE ( * , * )'ELEMENT', L,'NODE ',L
            !       WRITE ( * , * )'SW=',SWG (1),'SL=',SLLG(1),'SI=',SIIG(1),'RELK=',RELKG (1)
            !END IF   
            !      
            !      
...
!===================================================================== Xinghua
         !IF (IT==ITMAX) THEN
         !   WRITE (234171,'(4E16.5)') VISCG  !PuLi-20200103
         !   WRITE (234172,'(4E16.5)') RHOG   !PuLi-20200103
         !ENDIF 
!===================================================================== Xinghua     
!.....CALCULATE VELOCITY AT ELEMENT CENTROID WHEN REQUIRED              
 ...
 2300    AXSUM = 0.0D0 
         ...
         DO 2400 KG = 1, 8 
                ...
                AZSUMD=AZSUMD+VZG(KG)*ESLLG    
             
            AXSUM = AXSUM + VXG (KG) 
            AZSUM = AZSUM + VZG (KG) 
2400     AYSUM = AYSUM + VYG (KG) 
         VDMAG(L)=DSQRT(AXSUMD*AXSUMD+AYSUMD*AYSUMD+AZSUMD*AZSUMD)              !!
