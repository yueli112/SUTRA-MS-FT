...
!                                                                       
      SUBROUTINE INDAT2 (PVEC, UVEC, PM1, UM1, UM2, CS1, CS2, CS3, SL,  &
                         ...
                         ,SII,SLL,DSLDP,DSLDT,DSIDP,DSIDT,SIOUT,SII1,CFREEZ)                      
      USE PARAMS 
      USE CONTRL 
   ...
   !   USE FRStorage         !!冻结
      ...
      real (DP) :: &
        ...
        CS3(NN),&
        RCIT(NN),&
        SW(NN),SLL(NN), DSLDP(NN),DSLDT(NN),SII(NN), SIOUT(NN),SII1(NN),CFREEZ(NN), DSIDP(NN),DSIDT(NN), &        !!!
        DSWDP(NN),&
        PBC(NBCN) 
...
!.....INPUT INITIAL CONDITIONS FOR WARM START (UNIT-fICS DATA)            
  500 READ (fICS,*,iostat=ios) TSTART, DELTP, DELTU 
      ...
      CALL ZERO (SLL, NN, 1.0D0)              !!
    CALL ZERO (SII, NN, 0.0D0)
    CALL ZERO (SIOUT, NN, 0.0D0)    
    CALL ZERO(DSLDP,NN,0.0D0)
    CALL ZERO(DSLDT,NN,0.0D0)
    CALL ZERO(DSIDP,NN,0.0D0)
    CALL ZERO(DSIDT,NN,0.0D0)
...
!.....INITIALIZE SATURATION, SW(I)                                      
    ...
    CALL ZERO (SLL, NN, 1.0D0)              !!
    CALL ZERO (SII, NN, 0.0D0)
    CALL ZERO (SII1, NN, 0.0D0)
    CALL ZERO(CFREEZ,NN,0.0D0)
    CALL ZERO (SIOUT, NN, 0.0D0)    
    CALL ZERO(DSLDP,NN,0.0D0)
    CALL ZERO(DSLDT,NN,0.0D0)
    CALL ZERO(DSIDP,NN,0.0D0)
  ...
  !UVEC(I,1)这里用的要求第一种U必须是温度
  
    
     
  900 CALL ALLSAT(SW(I),DSWDP(I),SLL(I),DSLDP(I),DSLDT(I),SII(I),DSIDP(I),DSIDT(I),RELK,PVEC(I),UVEC(I,1),NodeMap(I),SIOUT(I),UVEC(I,2),SII1(I),CFREEZ(I))
  990 CONTINUE
