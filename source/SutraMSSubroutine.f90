      ...
      !LOCAL VARIABLES
        INTEGER (I4B) :: &
         ...
         REAL (DP) :: &
          ...
          TSECU0, &
          CST, &
          CWT, SOLU,TURE,BINGRE,SHUIRE,TURE2,BINGRE2,SHUIRE2,QIANRE2,QIANRE,USDL,USDI,TOTICE,ZONGSHUI,ZONGBING,&      !!!
          BDELP1, &
          BDELU1, &
          BDELP, &
          BDELU, &
        ...
        REAL(DP):: TDLEVEL,SAL
        ALLOCATE (TOTWA(NN),SM(NN),TOTSA(NN))                     !!!!!!!!
        
        !        ********************           Area input-XXH      ******************************
   IF (IFEVAP.EQ.1)THEN   
       
    allocate (Sarea(NSOP))    !
    OPEN(1290999,FILE='SAREA.DAT',STATUS='UNKNOWN')      
	DO 1226 NWT=1,NSOP
      READ(1290999,*) Sarea(NWT)
...
!.....GET FIRST PRINT TIME FROM OUTPUT CONTROL                          
      ...
      ENDIF 
! OPEN(2002091,FILE='mn.DAT',STATUS='UNKNOWN')                                                                        
!                                                                       
...
!        BACKSUBSTITUTION ONLY ALLOWED FOR CASES WHERE NSPE.EQ.1 - SUTRA-MS 1.1                    
      IF ( MOD(IT - 1, NPCYC).NE.0 .AND. &
           ...
           IT.GT.2 .AND.                 &
           ITRMAX.EQ.1 .AND.             &
           NSPE.EQ.1 .AND.(IFREEZ.EQ.0)) THEN
         NOUMAT = 1                     
      END IF
...
!        PM1 (I) = PVEC (I) 
!      ENDFORALL 
!WRITE(30701,'(2I9,5E15.7)')IT,1,PM1(199),PITER(199),PVEC(199) 
      DO I = 1, NN
        PITER(I) = BDELP1 * PVEC(I) - BDELP * PM1(I) 
        PM1(I)   = PVEC(I) 
      END DO 
...
!                                                                       
      DO I = 1, NN
        ...
        RCITM1(I) = RCIT(I) 
      END DO
!WRITE(30701,'(2I9,5E15.7)')IT,5,PM1(199),PITER(199),PVEC(199) 
      !                                                                       
!.....SPECIFIED PRESSURE FLUXES                                         
!      FORALL (IP = 1:NPBC) 
!        QPLITR (IP) = GNUP * (SpecifiedPBC(IP)%P - PITER ( IABS ( SpecifiedPBC(IP)%node ) ) ) 
...
!   WRITE(30701,'(2I9,5E15.7)')IT,17,PM1(199),PITER(199),PVEC(199)       
...
3300 &
    ...
    END SELECT
!    WRITE(30701,'(2I9,5E15.7)')IT,18,PM1(199),PITER(199),PVEC(199) 
...
!         ENDFORALL 
         DO I = 1, NN
           UVEC(I, KSP) = CVEC(I) 
        !  IF(I==302)WRITE(*,*)'22', QIN(302)  
         END DO 
3400 CONTINUE 
!                                                                       
!.....SET TIME-DEPENDENT BOUNDARY CONDITIONS, SOURCES AND SINKS         
!        FOR THIS TIME STEP   
!WRITE(30701,'(2I9,5E15.7)')IT,16,PM1(199),PITER(199),PVEC(199) 
         IF (ITER.EQ.1.AND.IBCT.NE.4) &
           CALL BCTIME (IPBCT, IUBCT, IQSOPT, IQSOUT, GNUP, PM1, UM1,UM2, GNUPCJ, TDLEVEL, OUTSEEP) ! CHENGJI 2013-09-02                                                         
! 
...
!.....DO NODEWISE CALCULATIONS IN MATRIX EQUATION FOR P AND/OR U        
         CALL NODAL( U_ML(K) )
!
!   WRITE(30701,'(2I9,5E15.7)')IT,6,UM1(10100,2),UITER(10100,2),UVEC(10100,2)
         do I=1,NN
               RHOCHANGE(I)=  RHO(I)
        IF(NSPE==2) THEN
             UCHANGE(I)=UITER(I,2)
...
!!WRITE(*,*) '计算',UVEC(1,2),UITER (1,2)  !!UITERG更接近但不是最后结果
!WRITE(30701,'(2I9,5E15.7)')IT,8,PM1(10100),PITER(10100),PVEC(10100)                                                                     
         CALL CPU_TIME (RPTIME1) 
...
!.....SOLVE FOR P                                                       
    ...
    END SELECT
!
!!    WRITE(*,*) '计算2',UVEC(1,2),UITER (1,2)  !!不显示
 5000    KKK = 000000 
         KPU = 1 
         WRITE ( * , * ) '....SOLVE FOR P' 
!                                                                       
         CALL CPU_TIME (RPTIME0) 
! WRITE(30701,'(2I9,5E15.7)')IT,12,PM1(10100),PITER(10100),PVEC(10100) !                                                                       
         if(.not.SOLVER (KKK, KPU, KSOLVP, PMAT, PVEC, PITER, B, NN,       &
                         IHALFB, NELT, NCBI)) then
           ISTOP_OUTER = -1
           IGOI_OUTER  =  1
         end if
 !WRITE(30701,'(2I9,5E15.7)')IT,13,PM1(10100),PITER(10100),PVEC(10100) !                                                                       
         CALL CPU_TIME (RPTIME1) 
...
DO I = 1, NN
  ...
  IF (KSP.NE.NESP) THEN                                         !!!!!!!!!!!!!!salt precipitate
   DO I = 1, NN 
    
       IF (IT.EQ.1) THEN
         SM(I)=0                                               !!Initial solid salt mass SM
       ENDIF
  !!   IF (UVEC(I, KSP).LT.0) THEN                        !!  Set negative values caused by errors to zero.
  !!     UVEC(I, KSP)=0
  !!   ENDIF  
  !!   TOTICE(I)=RHOW0*VOL(I)*NodeData(NodeMap(i))%por*SII(I)
     TOTWA(I)=SLL(I)*VOL(I)*RCIT(I)*NodeData(NodeMap(i))%por      !!TOTWA Node Total Water Quality  
     TOTSA(I)=TOTWA(I)*UVEC(I, KSP)+ SM(I)         !!TOTSA Node Total Salt Mass
  !   !IF(TOTSA(I).GT.SOLUBILITY*TOTWA(I))THEN
  !   !  UVEC(I, KSP)= SOLUBILITY+(TOTSA(I)-SOLUBILITY*TOTWA(I))/(KSALT*TOTWA(I))   !!The slope of the salinity increase beyond the solubility limit is 1/KSALT.
  !   ! SM(I)=TOTSA(I)-UVEC(I, KSP)*TOTWA(I)
  !   !ELSEIF (TOTSA(I).LE.SOLUBILITY*TOTWA(I))THEN
  !   !   UVEC(I, KSP)= TOTSA(I)/TOTWA(I)
  !   !   SM(I)=0
  !   !END IF
  !   
  !      IF(TOTSA(I).GT.SOLUBILITY*TOTWA(I))THEN
  !     UVEC(I, KSP)= SOLUBILITY
  !    SM(I)=TOTSA(I)-UVEC(I, KSP)*TOTWA(I)
  !   ELSEIF (TOTSA(I).LE.SOLUBILITY*TOTWA(I))THEN
  !      UVEC(I, KSP)= TOTSA(I)/TOTWA(I)
  !      SM(I)=0
...
!     SOLUTE MASS BUDGET                                                
         IF (KBUDG.EQ.1) &
           CALL BUDGET (ML, IBCT, VOL, SW, DSWDP, SLL,DSLDP,DSLDT,SII,DSIDP,DSIDT,RHO,    &
                        QIN, PVEC, PM1, PBC, QPLITR, IPBC, IQSOP, UVEC,PITER,UITER, UM1, &
                        UM2, UIN, QUIN, IQSOU, UBC, IUBC, CS1, CS2, CS3, SL, SR, GNUPCJ,SIOUT,SII1,CFREEZ) ! CHENGJI 2013-09-02 
...
!
!=======================================================================yu   output P T SW C to file .m and .mn
      if (IT .EQ. 1) then  
      allocate (XNT(NN))
	  allocate (YNT(NN))
	  allocate (ZNT(NN))
      ...
      allocate (MEANSI(NN))        
         ...
         if (mod(IT,OUT_INTE).EQ. 1.0) then          !    print interval
...
!            YNT(I) = YNT(I) + Y (I)
!            ZNT(I) = ZNT(I) + Z (I)
            MEANP(I)= MEANP(I)+SLL(I)*0.4*UVEC(I,2)*RHO(I)
            MEANT(I)= MEANT(I)+UVEC(I,1)
			MEANS(I)= MEANS(I)+SW(I)
			MEANSL(I)= MEANSL(I)+SLL(I)
			MEANSI(I)= MEANSI(I)+SIOUT(I)            
            MEANC(I)= MEANC(I)+UVEC(I,2)

35761      CONTINUE         
         
        IF (mod(IT,8640)  .EQ. 0) THEN           !    steps over a tidal cycle
!	  WRITE (200209,*) 'VARIABLES = "X","Y", "Z", "p", "t","c", "SLL"'
 !     WRITE (200209,*)' Time step=',IT
!        IF (IT .GE. 2800 ) THEN   
        	DO 89689 I=1,NN          
	  !       write (200409,89980)IT,I, MEANP(I)/OUT_INTE, MEANT(I)/OUT_INTE, MEANS(I)/OUT_INTE, MEANSL(I)/OUT_INTE, MEANSI(I)/OUT_INTE, MEANC(I)/OUT_INTE
	 !        write (200209,89986) IT,I,PVEC(I), UVEC(I,1),SW (I), SLL(I),SII (I) ,SIOUT(I) , UVEC(I,2)           
89689       CONTINUE
!	   ENDIF
        ENDIF
  89980       FORMAT(2I,16E15.5e3)        
89986       FORMAT(2I,16E15.5e3)  
!=======================================================================yu       
  !    CALL OUTNODE2 (PVEC, UVEC, SW, SLL,SII,IN, X, Y, Z, TITLE1, TITLE2)
...
!      ENDIF
      ...
      IF (PRNNOD) CALL OUTNODE (PVEC, UVEC, SW,SLL,SII,SIOUT, IN, X, Y, Z, TITLE1, TITLE2)                                                           
      IF (PRNELE) CALL OUTVELOCITY(VMAG,VDMAG, VANG1, VANG2, IN, X, Y, Z, TITLE1, TITLE2)                                                        
      IF (PRNOBS) CALL OUTOBS(IOBS, X, Y, Z, PVEC, UVEC, SW, SLL,SII,TITLE1, TITLE2)                                                           
!.....VERSION 1.1                                                                       
...
!    
      ...
      !
      ! if(mod(IT,6).EQ. 0.0  )  then
      !     
      !   do JJII=1,NN
      !         write (2002091,8986)  PVEC(JJII), UVEC(JJII,1), SW(JJII), SLL(JJII), SII(JJII)
      !   enddo
      !endif  
    SOLU=0
    TURE=0
    ...
    ZONGBING=0
    ZONGSHUI=0
      if(mod(IT,3600).EQ. 0.0  )  then
           write (20250809,8986) IT,TSEC,(uvec(K,1),K=1,2001)     
           write (2025080902,8986) IT,TSEC,(uvec(K,1),K=4003,6003)  
           write (2025080903,8986) IT,TSEC,(uvec(K,1),K=12007,14007)  
      endif 
    
      if(mod(IT,20).EQ. 0.0  .OR.IT==1)  then
           write (20250811,8986) IT,minval(uvec(:,1))  ,sum(sll)   
      endif 
      
      if(mod(IT,360).EQ. 0.0.OR.IT==1  )  then
          write (20250816,8986) IT,TSEC,(uvec(K,1),K=1,3001)     
          write (20250816,8986) IT,TSEC,(uvec(K,2),K=1,3001)
          write (20250816,8986) IT,TSEC,(sii(K),K=1,3001)     
          write (20250816,8986) IT,TSEC,(sll(K),K=1,3001)     
          write (20250816,8986) IT,TSEC,(siout(K),K=1,3001)     
             
      endif 
      
        if(mod(IT,360).EQ. 0.0.OR.IT==1  )  then
          write (20250818,8986) IT,TSEC,(uvec(K,1),K=1,NN)     
          write (20250818,8986) IT,TSEC,(uvec(K,2),K=1,NN)
          write (20250818,8986) IT,TSEC,(sii(K),K=1,NN)     
          write (20250818,8986) IT,TSEC,(sll(K),K=1,NN)     
          write (20250818,8986) IT,TSEC,(siout(K),K=1,NN)     
             
      endif     
      
       if(mod(IT,360).EQ. 0.0.OR.IT==1  )  then
          write (20250822,8986) IT,TSEC,(uvec(K,2),K=1,NN)     
          write (20250822,8986) IT,TSEC,(uvec(K,2)*sll(K),K=1,NN)
          write (20250822,8986) IT,TSEC,sum(uvec(:,2)*sll(:))    
          write (20250822,8986) IT,TSEC,(uvec(K,1),K=1,NN)     
          write (20250822,8986) IT,TSEC,(sii(K)+sll(K)+siout(K),K=1,NN)     
     ...
     SII1(JJII)=SII(JJII)
      SW1(JJII)=SW(JJII)
      RHO1(JJII)=RCIT(JJII) 
       SIOUT2(JJII)=SIOUT1(JJII)
      SIOUT1(JJII)=SIOUT(JJII)
       UCHANGE1(JJII)=UCHANGE(JJII)
      QIN1(JJII)=QIN(JJII)
      QIN(JJII)=0
      UCHANGE(JJII)=0
      VOL1(JJII)=VOL(JJII)
