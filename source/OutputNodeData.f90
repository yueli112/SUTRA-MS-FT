...
!                                                                       
      SUBROUTINE OUTNODE(PVEC, UVEC, SW,SLL,SII,SIOUT, IN, X, Y, Z, TITLE1, TITLE2) 
      USE ITERAT 
      USE CONTRL 
      USE SOLVI 
      USE JCOLS 
      ...
      IMPLICIT NONE
      real (DP) :: &
        PVEC (NN), UVEC (NN, NSPE), SW (NN) , SLL (NN) , SII (NN) ,SIOUT(NN)
      real (DP) :: &
        X (NN), Y (NN), Z (NN) 
      real (DP) :: &
        VCOL (NCOLMX), VVAR (9 + NSPE-1)      !!
      CHARACTER(1) TITLE1 (80), TITLE2 (80) 
      CHARACTER(8) HORP 
      CHARACTER(13) TORC (NSPE) 
      CHARACTER(1) CPHORP, CPTORC, CPSATU ,CPSATL,CPSATI      !!CPSATL,CPSATI 
      integer (I4B) :: &
        IN (NIN), IIN (8)
      real (I4B) :: &
        TT (999999)
      integer (I4B) :: &
        ITT (999999), ISTORC (999999), ISHORP (999999), &
        ISSATU (999999) ,ISSATL(999999),ISSATI(999999)          !!
      LOGICAL PRINTN 
      !LOCAL VARIABLES
      CHARACTER (LEN=15) &
        TCHAR
      ...
      DO K = 6, (5 + NSPE) 
        ...
        COLTK5 (K)  = TCHAR
      ENDDO 
      COLTK5 (6 + NSPE)  = '  total Saturation' 
      COLTK5 (7 + NSPE)  = '  liquid Saturation' 
      COLTK5 (8 + NSPE)  = '  ice Saturation' 
      DO K = 1, (9 + NSPE-1) 
      ENDDO 
!                                                                       
...
!.....   Indicators of when variables are calculated and printed        
         ...
         DO 8 M = 1, NCOLS5 
            ...
            IF (J5COL (M) .EQ. (7 + NSPE-1)) CPSATU = 'Y' 
             IF (J5COL (M) .EQ. (8 + NSPE-1)) CPSATL = 'Y'               !!
             IF (J5COL (M) .EQ. (9 + NSPE-1)) CPSATI = 'Y'              !!
    8    END DO 
   10    CONTINUE 
         JT = JT + 1 
         ...
         IF (MOD (JT, NCOLPR) .EQ.0.OR.JT.EQ.1) THEN 
            ...
            ISTORC (KT) = LCTORC 
            ISSATU (KT) = LCHORP
             ISSATL(KT) = MAX(LCHORP,LCTORC)          !!
             ISSATI(KT) = MAX(LCHORP,LCTORC) 
...
!.....Write the header information                                      
         IF (fNOD.GT.0) THEN 
            ...
            ENDIF 
            WRITE (fNOD, 964) "NODEWISERESULTS", KTMAX, HORP, (TRIM(ADJUSTL(TORC(K))),K=1,NSPE), "ToSat" , "LiqSat", "IceSat"    !!                                    
            DO 20 KT = 1, KTMAX 
               WRITE (fNOD, 965) ITT (KT), TT (KT), CPHORP, ISHORP (KT),  &
               (CPTORC, ISTORC (KT), K = 1, NSPE), CPSATU, ISSATU (KT)  , CPSATL, ISSATL(KT), CPSATI, ISSATI(KT)               !!
   20       END DO 
         ENDIF 
  960 FORMAT   ("## ", 80A1,                                            &
     &         /"## ", 80A1,                                            &
...
!..... Nodewise Information repeated before each time step              
  ...
  966 FORMAT   ('## ',                                                  &
     ...
     &         /'## ', 77('='))                                         
         PRINTN = (J5COL (1) .EQ.1) 
         IF (PRINTN) THEN 
            WRITE (fNOD, 968) ( COLTK5 (J5COL (M) ), M = 1, NCOLS5) ,'SIOUT','SOLID SALT','TOTAL SALT'
  968 FORMAT       ("## ", 2X, A4, 19(A15)) 
         ELSE 
            WRITE (fNOD, 969) COLTK5 (J5COL (1) ) (3:15), &
                   ( COLTK5 (J5COL (M) ), M = 2, NCOLS5)  ,'SIOUT','SOLID SALT','TOTAL SALT'                                      
...
!.....  The nodewise data for this time step                            
  ...
  972       END DO 
            IF (PRINTN) THEN 
               WRITE (fNOD, 975) I, (VCOL (M), M = 2, NCOLS5) ,SIOUT(I),SM(I),TOTSA(I)
  975 FORMAT          (I9, 19(1PE15.7))               !PuLi-20201110 (I9, 19(1PE15.7))
            ELSE 
               WRITE (fNOD, 976) (VCOL (M), M = 1, NCOLS5) ,SIOUT(I),SM(I),TOTSA(I)
  976 FORMAT          (1X, 20(1PE15.7))                 !PuLi-20201110 (I9, 19(1PE15.7))
            ENDIF 
      ...
      !write (*,*) ' **************************************************'
      !  write (*,*) '   Saturation =',  SW(I) ,'Temperature  =', UVEC(I,1),'   Salinity  =', UVEC(I,2)     
      !write (*,*) ' **************************************************'
      !       END IF
  978    END DO 
      ENDIF 
