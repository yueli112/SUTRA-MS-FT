...
! 10 -- ELEMENT.DAT
   ...
   OPEN(20250809,FILE='ICENOD.DAT') 
      OPEN(2025080902,FILE='ICENOD2.DAT') 
         OPEN(2025080903,FILE='ICENOD3.DAT') 
    OPEN(20250811,FILE='TH2_MINITEP_liquid.DAT')   
   OPEN(20250816,FILE='cryosuction_all.DAT')  
      OPEN(20250818,FILE='cryosuction_all2.DAT')  
       OPEN(20250822,FILE='salt_distribution.DAT')  
   
       OPEN(1102,FILE='ALL_ENERGY_CHANGE_AND_BOUNDARY_FLUX.DAT')
       OPEN(1103,FILE='ALL_H2O_MASS_CHANGE_AND_BOUNDARY_FLUX.DAT')
    OPEN(1108,FILE='ALL_SOLUTE_CHANGE_AND_BOUNDARY_FLUX.DAT')
    OPEN(1109,FILE='TOTAL_WATER_AND_SALT_MASS.DAT')
...
! 
 ...
 if(BOUNDSPE.eq.1.or.BOUNDSPE.eq.2)then
...
elseif(BOUNDSPE.eq.3)then
     OPEN(2024327,FILE='surfacetem.txt',STATUS='UNKNOWN')   
     ALLOCATE (SURTEM1(6480)) 
    DO 1033 I=1,6480
        READ(2024327,*) SURTEM1(I)
1033 CONTINUE
     CLOSE(2024327)
     ...
     OPEN(012301,FILE='y_wd.FIL') 
       OPEN(012302,FILE='y_sl.FIL') 
        OPEN(012303,FILE='y_yd.FIL')       
...
!.....  SET ME= 0 FOR SOLUTE AND ENERGY TRANSPORT                  )   
 !   (SET IFREEZ=1 FOR FREEZING ENERGY TRANPORT, IFREEZ=0 OTHERWISE,这里将IFREEZ直接设置成1)
     ! IFREEZ = 1
      IALSAT = 1
      ...
      CALL PRSWDS(SIMSTR, 4, SIMULA, NWORDS)                      !PRSWDS是将第一个参数解析成字符串
!                                                                       
!                                                                       
...
!                                                                       
 2020 IF (KTYPE.LT.0) THEN 
      ...
      !check for consistency between NOBS and presence of fSOB(specified obs. locations)观测点
...
!        (READ UNIT-fICS DATA)                                            
      ...
      CALL INDAT2 (PVEC, UVEC, PM1, UM1, UM2, CS1, CS2, CS3, SL,  &
                         SR, RCIT, SW, DSWDP, PBC, IPBC, IPBCT,SII,SLL,DSLDP,DSLDT,DSIDP,DSIDT,SIOUT,SII1,CFREEZ)                          

  
    !  write (*,*) '   UVEC(2485,2) =', UVEC(2485,2)
