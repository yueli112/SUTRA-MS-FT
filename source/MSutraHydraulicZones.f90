...
! ***
! *** PURPOSE :                                                         
! ***  MODULE THAT IS USED TO PROCESS AQUIFER PARAMETER ZONE DATA        改了里面的节点相关计算，元素还没改
...
module SutraZoneModule
  ...
  logical function RdZoneData()
    ...
    !if zone file is not defined    没有zon文件就是每个节点都是一个区域
    if(fZON==0) then
      NodeZones=nn
      ElemZones=ne
      goto 1010
...
01010&
    ...
    ios=AddMemory(MemoryIndex('ZON'),'Vec',      5*NodeZones) !NodeData(NodeZones)       是因为tNodeData有4个，改5
    ios=AddMemory(MemoryIndex('ZON'),'Vec',     29*ElemZones) !ElemData(ElemZones), &     tElemData有29个
...
01020&
    ...
    backspace(fZON)
    read (fZON,*,iostat=ios) i, &
      NodeData(i)%por,NodeData(i)%compma,NodeData(i)%rhos              !读zon文件第二个data里的四个值
...
09999&
  ...
  logical function MkZoneSOP(COMPFL,COMPI)             !
    ...
    write(fLST,'(//,1x,a,/,132("-"),/,1x,a10,1x,4(a15,1x))') &
      'SOP for NODAL ZONES',&
      '      Zone','       Porosity','Matrix Compres.',' Fluid Compres.','LIQUID SpecificStorage','ICE SpecificStorage'    !

    do i=1,NodeZones
      !set sop for zone
      NodeData(i)%sop =(1.D0 - NodeData(i)%por ) * NodeData(i)%compma + NodeData(i)%por * COMPFL 
      NodeData(i)%sopi =(1.D0 - NodeData(i)%por ) * NodeData(i)%compma + NodeData(i)%por * COMPI      !
     
      !write out zone data
      write(fLST,'(1x,i10,1x,4(1PD15.5,1x))') &
        i, &
        NodeData(i)%por,NodeData(i)%compma,COMPFL,NodeData(i)%sop,NodeData(i)%sopi              !冰
