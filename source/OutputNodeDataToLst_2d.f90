!     SUBROUTINE        OUTPTCLST_2D           SUTRA-MS VERSION 2004.1
!                                                                       
! *** PURPOSE :                                                         
! ***  TO PRINT PRESSURE AND TEMPERATURE OR CONCENTRATION               
! ***  SOLUTIONS AND TO OUTPUT INFORMATION ON TIME STEP, ITERATIONS,    
! ***  SATURATIONS, AND FLUID VELOCITIES FOR 2-D PROBLEMS.              
! ***  OUTPUT IS TO UNIT fLST.                                            
!                                                                       
      SUBROUTINE OUTPTCLST_2D (ML, ISTOP, IGOI, PVEC, UVEC, VMAG, VANG, SW) 
!      USE ITERAT 
!      USE CONTRL 
!      USE PARAMS 
!      USE FUNITS
...
!        ML, ISTOP, IGOI
!      real (DP) :: &
!        PVEC (NN), UVEC (NN, NSPE), VMAG (NE), VANG (NE), SW (NN)                                                           
...
!     &   //2X,5(6X,'NODE',16X)/(2X,5(1X,I9,1X,1PD15.8)))                
!      IF (KVEL.EQ.1.AND.IT.GT.0) WRITE (fLST, 655) (L, VMAG (L), L = 1,   &
!      NE)                                                               
!      IF (KVEL.EQ.1.AND.IT.GT.0) WRITE (fLST, 656) (L, VANG (L), L = 1,   &
!      NE)                                                               
...
!     &   11X,'A N G L E   IN DEGREES FROM +X-AXIS TO FLOW DIRECTION ',  &
!     &   'AT CENTROID OF ELEMENT'//                                     &
!     &   2X,5(3X,'ELEMENT',16X)/(2X,5(1X,I9,1X,1PD15.8)))               
!      GOTO 700 
!                                                                       
...
!  735 FORMAT(///13X,'T  E  M  P  E  R  A  T  U  R  E'                   &
...
!      WRITE (fLST, 950) (L, VANG (L), L = 1, NE) 
