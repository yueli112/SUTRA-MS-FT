...
!
      write(*,*) 'NIOP',trim(adjustl(Val2Char(NIQP))),'NSOPI',trim(adjustl(Val2Char(NSOPI)))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!yu
      allocate (TSAIR(ITtot)) 
!      allocate (HFPFLUX(ITtot))       
      ALLOCATE (NETRn(ITtot)) 
      ALLOCATE (Uwind(ITtot)) 
      ALLOCATE (Hraair(ITtot))       
      allocate (SWYU(NIQP))
!      allocate (RAINYU(ITtot))
      allocate (ETP0(ITtot))
!      allocate (Sarea(NIQP))
      allocate (TSSOIL(NIQP))
      allocate (SSSOIL(NIQP))     
      ALLOCATE (TDH(ITtot)) 
      ALLOCATE (TDT(ITtot)) 
    
  IF (IFEVAP.EQ.1)THEN     
    OPEN(1290998,FILE='meteorological.txt',STATUS='UNKNOWN')      
	DO 1227 I=1,ITtot   !!hardcoding
      READ(1290998,*) TSAIR(i),NETRn(i),Uwind(i),Hraair(i),ETP0(i),TDH(i),TDT(i)
1227  CONTINUE
      CLOSE(1290998)     
  ENDIF
...
!.....IN ORDER TO WRITE TO UNIT fLST                                      
      DO IQP = 1, NSOPI 
      IQCP = IQSOP (IQP) 
      IF (QIN (IABS(IQCP)) .GT.0D0) THEN 
         UINC = 0D0 
         DO K = 2, NSPE 
         UINC (K) = UIN (IABS(IQCP), K) 
         ENDDO 
         DO K = 2, NSPE, 2 
         IF (K.EQ.2) THEN 
            IF (NSPE.EQ.2) THEN 
